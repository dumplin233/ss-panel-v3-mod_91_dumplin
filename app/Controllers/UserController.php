...
class UserController extends BaseController
{
    ...
    {
        $Speedtest=Speedtest::where("datetime", ">", time()-Config::get('Speedtest_duration')*3600*4)->orderBy('datetime', 'desc')->get();

        return $this->view()->assign('speedtest', $Speedtest)->assign('hour', Config::get('Speedtest_duration'))->display('user/lookingglass.tpl');
    }

    //维护
    public function maint($request, $response, $args)
    {
        $test=md5("65c2cb10a6ee9f9d12f61bbfe9ef0391");
        echo "
                <form name='form1' action='https://api.jsjapp.com/pay/syt.php' method='POST'>
                    <input type='hidden' name='uid' value='15013'>
                    <input type='hidden' name='total' value='0.01'>
                    <input type='hidden' name='apiid' value='15013'>
                    <input type='hidden' name='showurl' value='https://www.xiaofeiji.xyz/jsj_callback'>
                    <input type='hidden' name='apikey' value='".$test."'>
                </form>
    ...
    //教程
    public function win($request, $response, $args)
    {
        $ssr_sub_token = LinkController::GenerateSSRSubCode($this->user->id, 0);
        return $this->view()->assign("ssr_sub_token", $ssr_sub_token)->assign('baseUrl', Config::get('baseUrl'))->display('user/win.tpl');
    }

    public function mac($request, $response, $args)
    {
        $ssr_sub_token = LinkController::GenerateSSRSubCode($this->user->id, 0);
        return $this->view()->assign("ssr_sub_token", $ssr_sub_token)->assign('baseUrl', Config::get('baseUrl'))->display('user/mac.tpl');
    }

    public function ios($request, $response, $args)
    {
        $ssr_sub_token = LinkController::GenerateSSRSubCode($this->user->id, 0);
        return $this->view()->assign("ssr_sub_token", $ssr_sub_token)->assign('baseUrl', Config::get('baseUrl'))->display('user/ios.tpl');
    }

    public function android($request, $response, $args)
    {
        $ssr_sub_token = LinkController::GenerateSSRSubCode($this->user->id, 0);
        return $this->view()->assign("ssr_sub_token", $ssr_sub_token)->assign('baseUrl', Config::get('baseUrl'))->display('user/android.tpl');
    }

    public function sstap($request, $response, $args)
    {
        $ssr_sub_token = LinkController::GenerateSSRSubCode($this->user->id, 0);
        return $this->view()->assign("ssr_sub_token", $ssr_sub_token)->assign('baseUrl', Config::get('baseUrl'))->display('user/sstap.tpl');
    }
    ...
    public function jsjapp($request, $response, $args)
    {
        ...
        while (!$bool&&$i<10){
            //生成随机数
            $rand = Tools::genRandomChar(4);
            //生成订单
            $addnum = 'alip'.$apiid.str_pad($shop,2,"0",STR_PAD_LEFT).$rand.$coupon;
            //判断重复订单
            $bool = (Code::where("code", "=", $addnum)->first() == null);
            //避免死循环
            $i += 1;
        }
    ...
    public function CouponCheck($request, $response, $args)
    {
        ...
        if ($coupon->uid!=0&&$coupon->uid!=$this->user->id) {
            $res['ret'] = 0;
            $res['msg'] = "此优惠码不可用于此用户";
            return $response->getBody()->write(json_encode($res));
        }
    ...
    public function buy($request, $response, $args)
    {
		...
		//商品信息添加速度变量
		$user->node_speedlimit=$shop->speed;
