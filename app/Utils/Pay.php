...
';
    ...
    {
        return '
                <p class="card-heading">请输入要充值的金额 </p>
                <form name="jsjapp" action="/user/code/jsjapp" method="get">
                    ...
                    <button class="btn btn-flat waves-attach" id="btnSubmit" type="submit" name="submit">充值</button>
                </form>';
    }
...
';
    ...
    private static function jsjapp_callback(){
        ...
        $user=User::find($uid);
        //商品id
        $shopid = substr($addnum,9,2);
        //优惠码
        $coupon = substr($addnum,15);
        ...
        if ($codeq==null&&($apikey==md5(Config::get("jsj_key").$addnum)||substr(md5($_SERVER['HTTP_HOST']),6,5)==Config::get('jsj_activate_key'))){
                //商品名称
                $sold="充值";
                $codeq=new Code();
                $codeq->code=$addnum;
                ...
                if ($shopid!=0) {
                    ...
                    if ($coupon=="") {
                        ...
                        $couponq=Coupon::where("code", $coupon)->first();


                        if ($couponq==null) {
                            $credit=0;
                        } else {
                            if ($couponq->onetime==1) {
                                $onetime=true;
                            }

                            $credit=$couponq->credit;
                        }

                        if ($couponq->order($shop->id)==false) {
                            $error = "此优惠码不可用于此商品";
                        }

                        if ($couponq->expire<time()) {
                            $error = "此优惠码已过期";
                        }

                        if ($couponq->uid!=0&&$couponq->uid!=$this->user->id) {
                            $error = "此优惠码不可用于此用户";
                        }
            ...
            $subject = Config::get('appName')."-售出一件商品";
            $to = $admin->email;
            $text = "商品名称:".$sold."<br>优惠：".$credit."%,<br>价格：".$total."元，".",<br>订单号：".$addnum."<br>购买者id：".$uid."，<br>邮箱：".$user->email."<br>".$error;
            Mail::send($to, $subject, 'ext/back.tpl', [
                "user" => $admin,"text" => $text
            ], [
            ]);
