...
class Job
{
    ...
    public static function buchang()
        {
            $users = User::all();
    ...
    /*public static function buchang()
    {
        ...
        foreach ($users as $user) {
            if ($user->class==1){
                echo "user->id=".$user->id."   user->class=".$user->class."\n";
                echo "user->class_expire=".$user->class_expire;
                $user->class_expire=date("Y-m-d H:i:s", strtotime($user->class_expire)+30*86400);
                echo "   now user->class_expire=".$user->class_expire."\n";
                echo "\nuser->expire_in=".$user->expire_in;
                $user->expire_in=date("Y-m-d H:i:s", strtotime($user->expire_in)+30*86400);
                echo "   now user->expire_in=".$user->expire_in."\n\n";
                $user->save();
            }
